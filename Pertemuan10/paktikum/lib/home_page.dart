...
import 'package:paktikum/user.dart';
                ...
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        final res = await dataService.getUsers();
                        if (res != null) {
                          setState(() {
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        UserCreate? res = await dataService.postUser(postModel);
                        setState(() {
                          _result = res.toString();
                          usCreate = res;
                        });
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        final res = await dataService.putUser(
                          '3',
                          _nameCtl.text,
                          _jobCtl.text,
                        );
                  ...
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        final res = await dataService.deleteUser('4');
                        setState(() {
                          _result = res.toString();
                        });
                      },
              ...
              Row(
                children: [
                  Expanded(
                    child: ElevatedButton(
                      onPressed: () async {
                        final users = await dataService.getUserModel();
                        setState(() {
                          _users = users?.toList() ?? [];
                        });
                      },
  ...
  Widget hasilCard(BuildContext context) {
    return Column(
      children: [
        ...
        if (usUpdate != null) UserUpdateCard(usrUpdate: usUpdate!),
